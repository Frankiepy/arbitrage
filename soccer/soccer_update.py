...
def get_info(key):
    ...
    gc = gspread.service_account(filename='arbitrage_sheets/soccer/sheetsconnection-386902-8d39a29cfb3c.json')
    ...
    if odds_response.status_code != 200:
                ...
                for site2 in game['bookmakers']:
                    ...
                    if outcome < 0.99:
                        ...
                        try:
                            ...
                            gc = gspread.service_account(filename='arbitrage_sheets/soccer/spreadsheets2-387422-722bacd99d06.json')
                            sh = gc.open_by_key('1fblZM76XYfcG82Qel4xKiFnrVwyE_2D7UkV5XbdETFE')
                            worksheet = sh.sheet1
                            try:
                                worksheet.append_row(info1)
        ...
        start = []
        worksheet.update('M5', f"Remaining requests for last used API key: {int(odds_response.headers['x-requests-remaining'])}")
        worksheet.update('M6', f"Used requests for last used API key: {int(odds_response.headers['x-requests-used'])}")
        print('good')
        #info.append(start)
...
while True:
    time.sleep(1)
    update_val = worksheet.acell('M4').value
    if update_val == 'update' or update_val == 'Update' or update_val == 'UPDATE':
        print(get_info(worksheet.acell('M2').value))
        worksheet.update('M4', '')
